...
void Game::closeScene(Ref* pSender)
{
...
void Game::gameOver(bool isWinMode, bool isWin)
{
	...
	if (isWinMode)
	{
		auto show = Label::createWithTTF("Win!", "fonts\\arial.ttf", 72, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	...
	auto shows = Label::createWithTTF(StringUtils::format("Score:%d", _score),
		"fonts\\arial.ttf", 72, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	...
	auto showi = Label::createWithTTF(StringUtils::format("", _score),
		"fonts\\arial.ttf", 36, Size::ZERO, cocos2d::TextHAlignment::CENTER);
...
bool Game::init()
{
	...
	auto bg = Sprite::create("background_7.png");
	...
	auto closeItem = MenuItemImage::create(
		"back.png",
		"back_s.png",
		CC_CALLBACK_1(Game::closeScene, this));
	...
	//2.创建显示区，划分区域
	auto info = Sprite::create("status_board.png");
...
void Game::createHUD()
{
	...
	_showlevel = Label::createWithTTF(level_str, "fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::LEFT);
	_showlevel->setPosition(x, y - 75);
	this->addChild(_showlevel);

	auto tscore = Label::createWithTTF("Score:", "fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::LEFT);
	tscore->setColor(Color3B::BLACK);
	auto str2 = StringUtils::format("%d", _score);
	_showscore = Label::createWithTTF(str2, "fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	tscore->setPosition(x, y - 150);
	_showscore->setPosition(x, y - 200);
	this->addChild(tscore);
	this->addChild(_showscore);

	auto tlife = Label::createWithTTF("Life:", "fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::LEFT);
	tlife->setColor(Color3B::BLACK);
	auto str = StringUtils::format("%d", _life);
	_showlife = Label::createWithTTF(str, "fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	...
	addChild(news);*/
	_show_game_info = Label::createWithTTF("", "fonts\\arial.ttf", 24, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	...
	_show_point_multi = Label::createWithTTF(StringUtils::format("%d", _point_multiple),
		"fonts\\arial.ttf", 0, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	...
	_show_brickcount = Label::createWithTTF(StringUtils::format("%d/%d", brickcount, allbrickcount),
		"fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::CENTER);
		...
		if (SB->getTag() == BRICK_TAG && SA->getTag() == BALL_TAG)
		{
			...
			if (!brick->isunbreakable())
			{
				...
				if (brick->gethp() == 0)
				{
					...
					int px = brick->getPosition().x;
					int py = brick->getPosition().y;
					CCSprite * sprite = CCSprite::create("boom.png");
					sprite->setPosition(ccp(px, py));
					this->addChild(sprite);
					auto fadeOut = FadeOut::create(1);
					sprite->runAction(Sequence::create(fadeOut, NULL));
					if (brick->isdropitem())
					{
						int side = 1;
				...
				{
					LOG_INFO(StringUtils::format(" HP:%d/%d", brick->gethp(), brick->getMaxhp()));
				}
			}
...
void Game::collisionDetection(Plate* player)
{
	for (auto iter = V_items.begin(); iter != V_items.end(); )
	{
		...
		else if (player->getBoundingBox().intersectsRect(it->getBoundingBox()))
		{
			...
			{
				player->expand();
				LOG_INFO(StringUtils::format("Expand %.1f", player->getScaleX()));
				break;
			}
			case Shrink:
			{
				player->shrink();
				LOG_INFO(StringUtils::format("Shrink %.1f", player->getScaleX()));
				break;
			}
			case BallSpeedUp:
			{
				...
				LOG_INFO(StringUtils::format("SpeedUP\n %.1f", V_balls.back()->getSpeed()));
				break;
			}
			case BallSpeedDown:
			{
				...
				LOG_INFO(StringUtils::format("SpeedDOWN\n %.1f", V_balls.back()->getSpeed()));
				break;
			}
			case PlateSpeedUp:
			{
			...
			case BallLarger:
			{
				...
				LOG_INFO(StringUtils::format("Bigger %.1f", V_balls.back()->getScale()));
				break;
			}
			case BallSmaller:
			{
				...
				LOG_INFO(StringUtils::format("Smaller %.1f", V_balls.back()->getScale()));
				break;
			}
			case Points:
			{
...
void Game::writeScoreToUserData()
{
	...
	for (int i = 0; i < 10; i++)
	{
		string score = StringUtils::format("Normal_score_%d", i);
		string brick = StringUtils::format("Normal_brick_%d", i);
		string level = StringUtils::format("Normal_level_%d", i);
		int s;
	...
	int time = UserDefault::getInstance()->getIntegerForKey("Normal_played", 0);
	int average = UserDefault::getInstance()->getIntegerForKey("Normal_average", 0);
	int maxscore = UserDefault::getInstance()->getIntegerForKey("Normal_maxscore", 0);

	average = (average*time + finalscore) / (time + 1);
	time++;
	maxscore = maxscore > _score ? maxscore : _score;
	UserDefault::getInstance()->setIntegerForKey("Normal_played", time);
	UserDefault::getInstance()->setIntegerForKey("Normal_average", average);
	UserDefault::getInstance()->setIntegerForKey("Normal_maxscore", maxscore);
}
