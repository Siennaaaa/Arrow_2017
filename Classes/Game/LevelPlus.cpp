...
using namespace std;
Scene* LevelPlus::createScene()
//建立场景
{
	auto scene = Scene::create();
	auto layer = LevelPlus::create();
	scene->addChild(layer);
	return scene;
}

void LevelPlus::closeScene(Ref* pSender)
//关闭场景
{
...
void LevelPlus::addbrick(int type, float x, float y)
//添加砖块的函数
{
...
void LevelPlus::WritetoFile(cocos2d::Ref* pSender)
//写入文件函数
{
	bool rewrite = false;
	...
	outfile.close();
	if (rewrite == false)
		_showsavelog->setString("");
	else
		_showsavelog->setString("");
}
void LevelPlus::ReadfromFile(cocos2d::Ref* pSender)
//从文件中加载地图
{
	if (!FileUtils::getInstance()->isFileExist("D:/1/cocos_project/brickmaster/Resources/levels/user_levels/" + _fileName))
	{
...
//主函数
{
	...
	_showfileName = Label::createWithTTF(StringUtils::format("level_%d", _levelNum), "fonts\\arial.ttf", 36, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	_showfileName->setPosition(info->getContentSize().width / 2, 300);
	this->addChild(_showfileName);
	_showsavelog = Label::createWithTTF("(No File Saved)", "fonts\\arial.ttf", 24, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	_showsavelog->setPosition(info->getContentSize().width / 2, 75);
	this->addChild(_showsavelog);

	auto add = Label::createWithTTF("+", "fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	...
	auto minus = Label::createWithTTF("-", "fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	...
	auto closeItem = MenuItemImage::create(
		"CloseNormal.png",
		"CloseSelected.png",
		CC_CALLBACK_1(LevelPlus::closeScene, this));
	float x = origin.x + visibleSize.width - closeItem->getContentSize().width / 2;
	float y = origin.y + closeItem->getContentSize().height / 2;
	closeItem->setPosition(Vec2(x, y));

	auto loadlable = Label::createWithTTF("Load", "fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	auto loadItem = MenuItemLabel::create(loadlable,
		CC_CALLBACK_1(LevelPlus::ReadfromFile, this));
	loadItem->setPosition(info->getContentSize().width / 2, 225);

	auto savelable = Label::createWithTTF("Save", "fonts\\arial.ttf", 48, Size::ZERO, cocos2d::TextHAlignment::CENTER);
	auto saveItem = MenuItemLabel::create(savelable,
		CC_CALLBACK_1(LevelPlus::WritetoFile, this));
	...
	listener_func->onTouchBegan = CC_CALLBACK_2(LevelPlus::FuncButtonTouchBegan, this);
	auto listener_screen = EventListenerTouchOneByOne::create();
	listener_screen->onTouchBegan = CC_CALLBACK_2(LevelPlus::ScreenTouchBegan, this);
	listener_screen->onTouchMoved = CC_CALLBACK_2(LevelPlus::ScreenTouchMoved, this);
	listener_screen->onTouchEnded = CC_CALLBACK_2(LevelPlus::ScreenTouchEnd, this);
...
bool LevelPlus::FuncButtonTouchBegan(Touch* touch, Event* event)
//开始按钮
{
...
bool LevelPlus::ScreenTouchBegan(Touch* touch, Event* event)
//屏幕按钮
{
	Vec2 nodeLoca = this->convertToNodeSpace(touch->getLocation());
	switch (currenttag)
...
void LevelPlus::ScreenTouchMoved(Touch* touch, Event* event)
{
	Vec2 nodeLoca = this->convertToNodeSpace(touch->getLocation());
	if (!isformatmode)
		for (auto it : vec_brick)
...
void LevelPlus::ScreenTouchEnd(Touch* touch, Event* event)
{
